				...
				<div>
					...
					<table class="table table-bordered">
						...
						<tr ng-repeat="item in result.records" ng-cloak>
							...
							<td><a href="product/edit.html?productId={{:: item.productId}}">{{:: item.name }}</a></td>
							<td>{{:: item.sku }}</td>
							<td>{{:: item.categoryId }}</td>
							<td>{{:: item.enabled }}</td>
							<td><a href="" ng-click="showAttr=!showAttr">显示属性</a> <pre ng-show="showAttr" style="max-width: 800px;">{{:: item.attrs |json }}</pre></td>
							<td>
							<a href="quotation/edit.html?productId={{:: item.productId}}"><i class="al al-xinjian"></i>添加报价</a>
