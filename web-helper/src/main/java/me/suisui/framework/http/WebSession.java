package me.suisui.framework.http;
...
/**
 * Session实现， 主要是simple session的实现，为了便于扩展，把属性复制而不是集成过来
 * 
 * @author swaron
 * 
 */
public class WebSession implements Principal, ValidatingSession, Serializable {
	private static final long serialVersionUID = -6153801569840540153L;
	private transient static final Logger logger = LoggerFactory.getLogger(WebSession.class);
...
//	private Timestamp expireTime;
	...
	public WebSession() {
		this.timeout = 30 * MILLIS_PER_HOUR;
	...
	public void validate() throws InvalidSessionException {
		...
		if (isTimedOut()) {
			...
			String msg = "Session with id [" + sessionId + "] has expired. " + "Last access time: "
					...
					+ ".  Session timeout is set to " + timeout / MILLIS_PER_SECOND + " seconds (" + timeout
	...
	protected boolean onEquals(SimpleSession ss) {
		return (getStartTimestamp() != null ? getStartTimestamp().equals(ss.getStartTimestamp()) : ss
				.getStartTimestamp() == null)
				&& (getStopTimestamp() != null ? getStopTimestamp().equals(ss.getStopTimestamp()) : ss
						.getStopTimestamp() == null)
				&& (getLastAccessTime() != null ? getLastAccessTime().equals(ss.getLastAccessTime()) : ss
